...
from implementation.src.scripts_tar.measures.eval_measures_2018 import UtilityBasedMeasure, AreaBasedMeasures, MAPBasedMeasures
from implementation.src.scripts_tar.measures.eval_measures_2018 import DescriptionMeasures, CountBasedMeasures, GainBasedMeasures
from implementation.src.scripts_tar.measures.eval_measures_2018 import LossBasedMeasures, RecallBasedMeasures
...
class TarAggRuler(object):
    ...
    def finalize(self):
        # begin: modified code to return the metrics as dict and not just to print them
        metric_dict = {}
            ...
            for i in range(0, len(self.agg_tar_ruler.measures)):
                ...
                for (output,fmt) in measure.outputs.items():
                    if fmt == 1:
                        ...
                        metric_dict[output] = getattr(measure, output)
                    if fmt == 2:
                        ...
                        setattr(measure, output, total)
                        metric_dict[output] = getattr(measure, output)
                if type(measure) == GainBasedMeasures:
                    percent = 0
                    for i in range(0,measure.maxt):
                        x = 0.0
                        if measure.cg_max > 0.0:
                            x = float(measure.cgat[i])/float(measure.cg_max)
                        metric_dict["R@{0}%".format(int(percent+(100/measure.maxt)))] = x
                        percent += (100/measure.maxt)
        return metric_dict
        # end: modified code
...
class TarRuler(object):
    ...
    def finalize(self):
        ...
        for measure in self.measures:
            measure.finalize()
            for (output,fmt) in measure.outputs.items():
                measure_dict[output] = getattr(measure,output)
            if type(measure) == GainBasedMeasures:
                percent = 0
                for i in range(0,measure.maxt):
                    x = 0.0
                    if measure.cg_max > 0.0:
                        x = float(measure.cgat[i])/float(measure.cg_max)
                    measure_dict["R@{0}%".format(int(percent+(100/measure.maxt)))] = x
                    percent += (100/measure.maxt)
        return measure_dict
        # end: modified code
