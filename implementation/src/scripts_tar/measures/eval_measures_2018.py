...
class GainBasedMeasures(DescriptionMeasures):
    ...
    def __init__(self, topic_id, num_docs, num_rels):
        ...
        # begin: modified code (added variable for TNR@95%)
        self.tnr95 = 0.0

        self.outputs = {'cg_total':1, 'cg_max':1, 'cgat': 2,
                        'threshold':1,  'cg_threshold':1, 'ncg_threshold':1, 'tnr95': 1}
        # end: modified code
    ...
    def update_all(self, judgment, value):
        ...
        # begin: modified code
        if self.cg_total / self.cg_max >= 0.95 and (self.cg_total - 1) / self.cg_max < 0.95 and self.tnr95 == 0.0:
            fp = self.last_rank - self.cg_total
            tn = self.num_docs - self.last_rank - (self.cg_max - self.cg_total)
            self.tnr95 = tn / (tn + fp)
        # end: modified code
